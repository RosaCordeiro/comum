...
global type w_ge473_interface_sap from dc_w_2tab_consulta_selecao_lista_det
string tag = "w_ge473_interface_sap"
integer width = 3927
integer height = 2416
...
public function boolean wf_carrega_detalhe (long pl_nr_controle, ref dc_uo_dw_base pdw, ref string as_log)
end prototypes

public function boolean wf_processa ();String ls_Log, ls_Chave, ls_nm_tabela

Long ll_Linhas, ll_Linha, ll_Controle, ll_Tabela, ll_tabela_ant, ll_linhas_filtro, ll_progresso_1, ll_progresso_2=0, ll_qt_tabelas, ll_id_tipo
int  li_ret
...
if tab_1.TabPage_1.dw_2.rowscopy(1,tab_1.TabPage_1.dw_2.rowcount(), primary!, lds_dados, 1, primary!) < 0 then
	messagebox('Aten$$HEX2$$e700e300$$ENDHEX$$o','Problemas ao copiar dados (Rowscopy) da datawindow "dw_ge473_lista".')
	return false
end if
...
lds_dados.setsort('cd_tabela A, nr_controle A')
lds_dados.sort()
...
try
	...
	lds_dados.setfilter('id_processa = "S"')
	lds_dados.filter()
	...
	If ll_Linhas > 0 Then 
		...
		ll_qt_tabelas = tab_1.TabPage_1.dw_2.object.c_total_selecionados[tab_1.TabPage_1.dw_2.rowcount()]
		
		if ll_qt_tabelas > 0 Then
			Open(w_Aguarde_3)	
			
			w_aguarde_3.uo_progress.of_setmax(ll_qt_tabelas)
		end if
		
		For ll_Linha =1 To ll_Linhas
			
			If lds_dados.Object.id_processa[ll_Linha] = 'S' and (lds_dados.Object.id_situacao[ll_Linha]  = 'C' or lds_dados.Object.id_situacao[ll_Linha]  = 'E') Then
				
				ll_Controle	=lds_dados.Object.nr_controle[ll_Linha]
				ll_Tabela		= lds_dados.Object.cd_tabela[ll_Linha]
				ls_Chave		= lds_dados.Object.de_chave_sap[ll_Linha]
				ls_nm_tabela = lds_dados.Object.de_tabela[ll_Linha]

				w_aguarde_3.wf_settext('Interface: ' + ls_nm_tabela , 1)
				w_aguarde_3.wf_settext('Controle: ' + String(ll_controle) , 2)
				...
				Choose Case ll_Tabela
					...
					Case Else
						...
						if luo_exportacao.uf_executar( ll_tabela, ll_controle, ref ls_log) = false then
			...
			w_aguarde_3.uo_progress.of_setprogress(ll_progresso_2)
...
//		w_aguarde_3.uo_progress.
		...
		ElseIf ls_Tipo_Cliente = "CH" Then  //Cliente Cheque
			If (LenA(Trim(ls_CPF_CGC)) <> 11 and LenA(Trim(ls_CPF_CGC)) <> 14) or Isnull(ls_CPF_CGC) or Trim(ls_CPF_CGC) = ""  Then
				as_Log	= "O campo 'nr_cpf_cgc' est$$HEX2$$e1002000$$ENDHEX$$com o tamanho diferente do tamanho esperado de 11 ou de 14 caracteres. N$$HEX1$$fa00$$ENDHEX$$mero de controle "+String(al_controle)+"."
				Return False
			End If
...
public function boolean wf_verifica_obrigatoriedade_campo (long al_controle, string as_campo, string as_obrigatoriedade, string as_valor, ref string as_log);If as_obrigatoriedade = 'S' Then
	If IsNull(as_valor) or Trim(as_valor) = "" Then
		as_log = "Preechimento do campo '"+as_campo+"' $$HEX2$$e9002000$$ENDHEX$$obrigat$$HEX1$$f300$$ENDHEX$$rio"
		Return False
	End If
End If
...
public function boolean wf_carrega_detalhe (long pl_nr_controle, ref dc_uo_dw_base pdw, ref string as_log);Long ll_linhas, ll_for
string ls_log, ls_sql, ls_Erro_Create_DS, ls_Create_DS, ls_cor, ls_band, ls_objetos[], ls_x, ls_y, ls_h, ls_w

uo_ge473_comum lo_Comum
lo_Comum = Create uo_ge473_comum

ls_cor = tab_1.tabpage_1.dw_2.describe('datawindow.color')

lo_Comum.of_processa_carrega_dados(pl_nr_controle , ref ls_Log) 

ll_Linhas = lo_Comum.ids_lista_registros.RowCount()

if ll_linhas > 0 then
	
	 pdw.Settransobject(lo_Comum.io_SqlCa)
	
	ls_Create_DS = lo_Comum.io_SqlCa.SyntaxFromSQL("SELECT * FROM #temptab1 ",  "style(type=grid) Datawindow(Color=" + ls_cor + ")", ls_Erro_Create_DS)

	IF Len(ls_Erro_Create_DS) > 0 THEN
		as_log = "Erro SyntaxFromSQL '" + ls_Erro_Create_DS+ "'. Objeto [w_ge473_interface_sap], fun$$HEX2$$e700e300$$ENDHEX$$o [wf_carrega_detalhe]."
		Return False
	END IF
	
	pdw.Create( ls_Create_DS, ls_Erro_Create_DS)
	...
	if lo_Comum.ids_lista_registros.rowscopy( 1, lo_Comum.ids_lista_registros.rowcount(), Primary!, pdw, 1, Primary!) = -1 then
		as_log = "Erro ao executar RowsCopy. Objeto [w_ge473_interface_sap], fun$$HEX2$$e700e300$$ENDHEX$$o [wf_carrega_detalhe]."
		Return False
	end if
end if

for ll_for = 1 to long(pdw.object.datawindow.column.count)
	
	pdw.modify('#' + string(ll_for) + '.edit.displayonly=yes')
...
If  gvo_Aplicacao.ivo_Seguranca.Cd_Sistema <> "EX" Then
	Choose Case gvo_Aplicacao.ivo_Seguranca.Cd_Sistema 
		...
		Case "WS"; ll_Tabela_Defaut = 101 // recebimento da confirma$$HEX2$$e700e300$$ENDHEX$$o da cria$$HEX2$$e700e300$$ENDHEX$$o da migo e miro
	End Choose
...
io_ge543_tabela_interface_sap.is_tipo	= 'D'

ls_sistema	= gvo_aplicacao.ivo_seguranca.cd_sistema
//
If ls_sistema = 'EX' or ls_sistema = 'CO' or ls_sistema = 'WS' Then
	SetNull(ls_sistema)
End If
...
event tab_1::selectionchanged;Long ll_nr_controle_filho, ll_cd_tabela, ll_linha, ll_cd_tabela_pai, ll_nr_controle_pai, ll_linhas
string ls_log
...
if ll_linha > 0 Then
	...
	tabpage_2.dw_5.reset()
	
	ll_nr_controle_pai = tabpage_1.dw_2.object.nr_controle[ll_linha]
	
	ll_linhas = tabpage_2.dw_3.retrieve(ll_nr_controle_pai)
	
	if ll_linhas > 0 Then
	
		ll_nr_controle_filho = tabpage_2.dw_3.object.nr_controle[1]
	
		If Not parent.wf_carrega_detalhe( ll_nr_controle_filho, ref tabpage_2.dw_5, ref ls_log) Then
			MessageBox("Erro", ls_log)
			Return 0
		End if
		
		tabpage_2.dw_5.visible = True
	else
		tabpage_2.dw_5.visible = False
	end if
...
ll_id_tipo		= dw_1.Object.id_tipo			[1]
...
//	This.of_AppendWhere_SubQuery("ia.cd_sistema = '"+ls_cd_sistema+"'", ll_Index_SubQuery)
//End If
Choose Case gvo_Aplicacao.ivo_Seguranca.Cd_Sistema
	Case 'CR'
		This.of_AppendWhere_SubQuery("ia.cd_sistema = '"+gvo_Aplicacao.ivo_Seguranca.Cd_Sistema+"'", ll_Index_SubQuery)
	Case Else
		//
End Choose
...
If Not IsNull(ls_Ch_Legado)  and Trim(ls_Ch_Legado) <> '' Then
	This.of_AppendWhere_SubQuery(" exists (select * from interface_sap_item it " +&
												...
												"and cast(it.vl_item as varchar) like '%" + ls_Ch_Legado + "%')", ll_Index_SubQuery)
End If

if ll_id_tipo > 0 Then
	
	This.of_AppendWhere_SubQuery("i.de_chave_sap in (select ap.de_chave_sap from a_sergio_promocao ap " +&
												"where ap.id_controle = " + String(ll_id_tipo) + ")", ll_Index_SubQuery)
...
event dw_2::rowfocuschanged;call super::rowfocuschanged;If currentRow > 0 and Tab_1.TabPage_1.dw_4.RowCount() > 0 Then
...
event dw_3::rowfocuschanged;call super::rowfocuschanged;string ls_log

If currentRow > 0 Then
	wf_carrega_detalhe(This.Object.nr_controle[currentRow], ref dw_5, ref ls_log)
End If
end event
...
If Not gvo_Aplicacao.ivo_Seguranca.of_Libera_Acesso_Procedimento("W_GE473_EXECUTA_INTERFACE", ref ivs_Operador) Then 
	Return
End If
...
If Not gvo_Aplicacao.ivo_Seguranca.of_Libera_Acesso_Procedimento("W_GE473_CANCELA_INTERFACE", ref ivs_Operador) Then 
	Return
End If
...
st_log.cd_chave 				= '63581fde56024cb8a8b3c065c72a29ce'
...
st_log.nr_nf						= 171326
st_log.cd_fornecedor			= '053400532'
st_log.de_especie				= 'NFE'
st_log.de_serie					= '1'
st_log.cd_varchar				= 'SEGREG RECEB'

If Not gf_insere_log_exportacao_sap(st_log, ref ls_log) then Return 
...
event clicked;Long ll_Controle

String ls_MSG

ll_Controle	= tab_1.TabPage_1.dw_2.Object.nr_controle[tab_1.TabPage_1.dw_2.GetRow()]

If MessageBox('Aten$$HEX2$$e700e300$$ENDHEX$$o', 'Confirma o reprocessamento do controle [' + STring(ll_Controle) + '] ?', Question!, YesNo!, 2) = 2 Then Return 

Update interface_sap
Set id_situacao = 'C', dh_processamento = null
Where nr_controle = :ll_Controle
Using SqlCa;
...
long textcolor = 8388608
long backcolor = 67108864
string text = "Secund$$HEX1$$e100$$ENDHEX$$rio"
...
event clicked;tab_1.tabpage_2.dw_5.saveas()
end event
