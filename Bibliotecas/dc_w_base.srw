...
public function boolean wf_informacoes_historico (ref string ps_tabelas, ref string ps_primarykey, ref string ps_chave)
end prototypes
...
//Verifica se no grupo de acesso a tela est$$HEX2$$e1002000$$ENDHEX$$sem permiss$$HEX1$$e300$$ENDHEX$$o de altera$$HEX2$$e700e300$$ENDHEX$$o
...
If gvo_Aplicacao.ivo_Seguranca.cd_Sistema = 'RL' Then
	...
	Choose Case ls_ClassName
		...
		Case Else
			/* Atualmente o perfil com menos acesso $$HEX2$$e9002000$$ENDHEX$$o 12 - ESTAGI$$HEX1$$c100$$ENDHEX$$RIO (T$$HEX1$$c900$$ENDHEX$$CNICO EM FARM$$HEX1$$c100$$ENDHEX$$CIA), portanto, se n$$HEX1$$e300$$ENDHEX$$o existir
			 * procedimento cadastrado para o perfil 12, ser$$HEX2$$e1002000$$ENDHEX$$exigido login para verificar se o perfil possui acesso ao procedimento.
...
// Tenta executar o evento no objeto que est$$HEX2$$e1002000$$ENDHEX$$com o foco
lgo_Focus = GetFocus()
...
public function boolean wf_informacoes_historico (ref string ps_tabelas, ref string ps_primarykey, ref string ps_chave);String lvs_Tabela
String lvs_Campo
...
Try
	...
	ivds_1.Of_ChangeDataObject("dw_primary_key_matriz")
	
	For lvi_Contador = 1 To UpperBound(ivo_Update_DW)
		//Pega DW
		lvo_DW = ivo_Update_DW[lvi_Contador]
		//Captura qual a tabela est$$HEX2$$e1002000$$ENDHEX$$marcada no UpdateProprieties
		lvs_Tabela = lvo_DW.Describe("DataWindow.Table.UpdateTable")
		
		//Possui uma tabela atualiz$$HEX1$$e100$$ENDHEX$$vel?
		If lvs_Tabela <> "?" and lvs_Tabela <> "" Then
			//$$HEX2$$c9002000$$ENDHEX$$a primeira tabela na ordem de salvamento?
			If ps_tabelas = "" Then
				//Recupera o nome dos campos que comp$$HEX1$$f500$$ENDHEX$$em a PK
				lvl_Chaves = ivds_1.Retrieve(lvs_Tabela)
				//Para cada campo chave da PK
				For lvl_Chave = 1 To lvl_Chaves
					//Pega os campos que compoem a chave
					lvs_Campo 		= ivds_1.Object.name [lvl_Chave]
					//Guarda os campos da primarykey
					ps_primarykey	+= IIF(ps_primarykey<>"", ", ", '')+ps_primarykey
					//Se a DW tiver registro
					If lvo_DW.RowCount() > 0 Then
						//Recupera o tipo de dado da coluna
						lvs_ColunmType = Upper(lvo_DW.Describe(lvs_Campo+".Coltype"))
						lvs_Edit = Lower(lvo_DW.Describe(lvs_Campo+".Edit.Style"))
						//Recupera o dado da coluna conforme o tipo e converte para string
						Choose Case lvs_ColunmType
							Case 'NUMBER','LONG','INT','ULONG'
								lvs_Chave += String(lvo_DW.GetItemNumber(1,lvs_Campo))
							Case 'DATE'
								lvs_Chave += String(lvo_DW.GetItemDate(1,lvs_Campo))
							Case 'DATETIME'
								lvs_Chave += String(lvo_DW.GetItemDateTime(1,lvs_Campo))
							Case Else	
								If (Pos(lvs_ColunmType,'DECIMAL')>0)or(lvs_ColunmType='REAL') Then
									lvs_Chave += String(lvo_DW.GetItemDecimal(1,lvs_Campo),'#,##0.00##')
								Else
									lvs_Aux		=	lvo_DW.GetItemFormattedString(1,lvs_Campo)
									If IsNull(lvs_Aux) or Trim(lvs_Aux)='' or (lvs_Edit='ddlb') Then lvs_Aux = lvo_DW.GetItemString(1,lvs_Campo)
									lvs_Chave	+= lvs_Aux
								End If
						End Choose			
						//Guarda a informa$$HEX2$$e700e300$$ENDHEX$$o para o retorno
						ps_chave 		+= IIF(ps_chave<>"", " - ", "") + lvs_Chave	
					End If
				Next
			End If
			//Monta o retorno das tabelas
			ps_tabelas += IIF(ps_tabelas<>'', ', ', '')+lvs_Tabela
		End If
	Next
...
Finally
	If IsValid(ivds_1) Then Destroy(ivds_1)
End Try
...
event activate;If Pos(Upper(gvo_Aplicacao.of_UserId()), "COLETOR") > 0   Then
