...
Try
	 ...
	 *Essa regra n$$HEX1$$e300$$ENDHEX$$o $$HEX2$$e9002000$$ENDHEX$$utilizada para a filial 2. Essa filial fatura para v$$HEX1$$e100$$ENDHEX$$rios centros de custo impossibilitando o agrupamento.
	...
	If ll_Linhas_Filial > 0 Then
		For ll_Linha_Filial = 1 To ll_Linhas_Filial			
			...
			If ll_Filial_Lista = 534 And ll_Centro_Custo_Lista = 100896 And ll_Centro_Custo_Entrega_Lista = 100896 Then 
				...
				//Variavel para controlar os caminhos a seguir se $$HEX2$$e9002000$$ENDHEX$$pedido almox cd ou o padr$$HEX1$$e300$$ENDHEX$$o que ja era nas versoes anteriores
				lb_Almox_Cd = True
			Else
					...
					If of_Insere_Pedido_Distribuidora(ll_Filial, ldc_Valor_Pedido, ll_Centro_Custo_Entrega, Ref ll_Proximo_Pedido_Distribuidora) Then
						...
						If lb_Almox_Cd Then
							...
							If of_Insere_Pedido_Distribuidora_Almox_Cd(ll_Filial, ll_Proximo_Pedido_Distribuidora,ll_Centro_Custo, ll_Centro_Custo_Entrega, ls_Tipo_Pedido, Ref ll_Cd_Pedido_Almox_Cd) Then
								...
								End If
							End If
						//Nao $$HEX2$$e9002000$$ENDHEX$$pedido almox do CD, faz o que fazia anteriormente/sem altera$$HEX2$$e700f500$$ENDHEX$$es
						Else
	...
	If ll_Linhas > 0 Then
		For ll_Linha = 1 To ll_Linhas
			...
			If al_Filial <> 2 Then
				...
				If SqlCa.SqlCode = -1 Then
					ls_Erro = "Filial: "+String(al_Filial)+"~rErro ao verificar se j$$HEX2$$e1002000$$ENDHEX$$existe o produto na tabela 'pedido_distribuidora_produto'." + SqlCa.sqlErrText
					SqlCa.of_Rollback()
					of_email_log(ls_Erro)
					Return False
				End If
...
////Ir$$HEX2$$e1002000$$ENDHEX$$inserir pedido somente para as filiais 2 e 809 ou para as filiais que tiverem o centro de custos preenchido
...
Try
	...
	If ll_Linhas > 0 Then
		...
		For ll_Linha = 1 To ll_Linhas
			...
			If SqlCa.SqlCode = -1 Then
				as_Erro = "Filial: "+String(al_Filial)+" Pedido "+String(al_pedido)+" ~rErro ao verificar se j$$HEX2$$e1002000$$ENDHEX$$tem o produto na tabela 'pedido_almoxarifado_produto':" + SqlCa.sqlErrText
				SqlCa.of_Rollback()
				Return False
			End If
...
//--------------Verifica se j$$HEX2$$e1002000$$ENDHEX$$tem um pedido distribuidora para pedidos de almoxarifado--------------
If al_Filial <> 2 And (IsNull(al_Centro_Custo_Entrega) Or (al_Centro_Custo_Entrega = 0) or al_Centro_Custo_Entrega = al_Filial) Then
	...
	Choose Case SqlCa.SqlCode
		...
		Case -1
			ls_Erro = "Filial: "+String(al_Filial)+"~rErro ao verificar se j$$HEX2$$e1002000$$ENDHEX$$existe um pedido distribuidora para os pedidos de almoxarifado:" + SqlCa.sqlErrText
			SqlCa.of_Rollback()
			of_email_log(ls_Erro)
			Return False
	End Choose
