...
//Verifica se $$HEX2$$e9002000$$ENDHEX$$uma nota da CLAMED
Select Count(1)
Into :ll_Qtde
From filial
...
If SqlCa.Sqlcode = -1 Then
	SqlCa.of_msgdberror("Erro ao verificar se a nota $$HEX2$$e9002000$$ENDHEX$$da CLAMED." )
	Return False
End If
...
Try
	...
	If Upper(Right(ls_Arquivo, 3))="ZIP" Then
		//Verifica se objeto ZIP j$$HEX2$$e1002000$$ENDHEX$$est$$HEX2$$e1002000$$ENDHEX$$criado
		If Not IsValid(ivo_ZIP) Then ivo_ZIP = Create dc_uo_zip
		
		//Novo arquivo XML
		ls_Arquivo_Aux = Mid(ls_Arquivo, 1, Len(ls_Arquivo) -4)
		
		//Se o arquivo ZIP j$$HEX2$$e1002000$$ENDHEX$$existir ele exclui
...
If Trim(dw_1.Object.de_chave_nota[1]) <> "" Then
	...
	If wf_Busca_Xml_Ftp(ls_Chave, ldt_Emissao, ls_DirXML, ref ls_ArquivoXML) Then
		If Run(ls_dirExec + " "+ ls_ArquivoXML) = -1 Then
			MessageBox("Aviso", "Erro ao abrir a nota.~rO arquivo est$$HEX2$$e1002000$$ENDHEX$$no diret$$HEX1$$f300$$ENDHEX$$rio: "+ ls_ArquivoXML+".")
		End If
	Else
