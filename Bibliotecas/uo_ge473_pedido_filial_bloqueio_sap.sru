...
String	is_cd_filial_sap,&
		...
		is_id_bloq_forn, is_id_desbloqueia
...
public function boolean of_atualiza_pedido_filial_bloqueio (long al_controle, long al_tabela)
end prototypes
...
public function boolean of_valida_dados (ref string as_log);Try
	...
	If IsNull(idh_dh_bloqueio) or Not IsDate(String(idh_dh_bloqueio, "dd/mm/yyyy")) Then
		as_Log	= "A data de bloqueio n$$HEX1$$e300$$ENDHEX$$o foi preenchida ou est$$HEX2$$e1002000$$ENDHEX$$com um valor inv$$HEX1$$e100$$ENDHEX$$lido."
		Return False
	End If
...
try 
	...
	If ll_Linhas > 0 Then
		For ll_Linha = 1 To ll_Linhas
			...
			uo_ge473_pedido_filial_bloqueio_sap  lo_Peido_Bloqueio_SAP
			 
			Try
				lo_Peido_Bloqueio_SAP	= Create uo_ge473_pedido_filial_bloqueio_sap
				lo_Peido_Bloqueio_SAP.of_atualiza_pedido_filial_bloqueio( lds.Object.nr_controle[ll_Linha] ,   il_Tabela)
			Finally
				Destroy(lo_Peido_Bloqueio_SAP)
			End Try			
			
		Next
	ElseIf ll_Linhas < 0 Then
		gvo_aplicacao.of_grava_log("Interface FilialBloqueioSAP - Erro ao recuperar os da DW [ds_ge473_lista_controles] - uo_ge473_pedido_filial_bloqueio.of_processa_atualizacao.")
	End If	
...
public function boolean of_atualiza_pedido_filial_bloqueio (long al_controle, long al_tabela);dc_uo_ds_base lds
